...
| [`experimento.contrato.ts`](src/lib/experimento.contrato.ts)                 | `RUTAS_EXPERIMENTO`: restablecer el entorno y entregar la traza de una ejecucion, con `a2a` (saltos, estados, artefactos) en B2/B3. Las consume el ejecutor, NO el frontend, y viven fuera de `/api` (decisiones 32 y 44)                             |
| [`mcp.contrato.ts`](src/lib/mcp.contrato.ts)                                 | `RUTA_MCP`, `CABECERA_TRACE_ID`, `CABECERA_AGENT_ID` con `AGENTES_MCP` y `PERMISOS_AGENTE` (HU-20), y las claves `_meta` que `mcp-server` y los agentes MCP (B1, B2, B3) leen igual (decision 42)                                                     |
| [`a2a.contrato.ts`](src/lib/a2a.contrato.ts)                                 | `RUTA_AGENT_CARD`, `AgentCardDto`, ciclo de vida de tareas A2A, JSON-RPC (`message/send`, codigos), `HABILIDADES_A2A`, `SolicitudEspecialistaDto`, `MedicionReceptorDto` (`unihelp/medicion`) y los artefactos de docs/03 (HU-29, HU-30, decision 44) |
