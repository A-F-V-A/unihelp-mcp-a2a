...
El nucleo del **agente unico** que comparten B0 y B1: bucle de function calling,
...
| Archivo                                                                                            | Contenido                                                                                         |
| -------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| [`agente-nucleo.module.ts`](src/lib/agente-nucleo.module.ts)                                       | `AgenteNucleoModule.forRoot({ identidad, imports, puertoCapacidades })`.                          |
| [`identidad-agente.ts`](src/lib/identidad-agente.ts)                                               | Servicio, protocolo y actor de auditoria; va a `tool_calls[].agente` y `transporte`.              |
...
| [`agente/instrumentador-trazas.ts`](src/lib/agente/instrumentador-trazas.ts)                       | Acumula tiempos, tokens y `tool_calls[]` por ejecucion (HU-34); no calcula metricas (RM-02).      |
...
| [`agente/extractor-objeto-final.ts`](src/lib/agente/extractor-objeto-final.ts)                     | Separa el texto para la persona del `resultado_triaje` (HU-30).                                   |
