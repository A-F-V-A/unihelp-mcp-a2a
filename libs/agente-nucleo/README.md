...
| [`configuracion/configuracion-agente.ts`](src/lib/configuracion/configuracion-agente.ts)           | Proveedor (`openai` u `ollama`, decision 46), modelo, URL base, muestreo, modo de casetes y limites, leidos del entorno; falla al arrancar si falta algo.   |
| [`modelo/cliente-modelo.ts`](src/lib/modelo/cliente-modelo.ts)                                     | Una peticion por Chat Completions (OpenAI o Ollama local), sin reintentos, sin paralelismo; mide `rtt` (M4.2).                                              |
