...
/**
 * ISO 8601 de una fecha que puede llegar como `Date` (caso de uso en proceso) o
 * ya serializada (resultado de una capacidad, que se normaliza como si hubiera
 * viajado por la red para que B0 y B1 vean la misma forma).
 */
export function aIso(fecha: Date | string): string {
...
export function aPropuestaTicketDto(propuesta: Propuesta, servicio: Servicio): PropuestaTicketDto {
  return {
    ...
    creadaEn: aIso(propuesta.creadaEn),
    resueltaEn: propuesta.resueltaEn === null ? null : aIso(propuesta.resueltaEn),
