...
import { aFunctionCalling } from './funcion-herramienta';
 ...
 * peticion sin reiniciar (HU-27). Se ejecutan de a una, en el orden emitido
 * (D1). El resultado vuelve en el ROL DE HERRAMIENTA, nunca pegado al mensaje de
 * la persona (capa 1 de la defensa ante inyeccion).
 */
@Injectable()
export class BucleAgente {
  constructor(
    @Inject(ClienteModelo) private readonly modelo: ClienteModelo,
    @Inject(PUERTO_CAPACIDADES) private readonly capacidades: PuertoCapacidades,
  ...
  async atender(
    ...
    for (;;) {
      ...
      const herramientas = aFunctionCalling(await this.capacidades.listar());
      let respuesta;
      try {
