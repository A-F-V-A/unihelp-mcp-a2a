...
| `UNIHELP_MODELO_PROVEEDOR`            | **[a crear]**        | —                              | Falla al arrancar                                                                    | RNF-01. `openai` u `ollama` (decisión 46); ambos por Chat Completions. Va a `model.provider`                                                                                                                     |
| `UNIHELP_MODELO_URL_BASE`             | **[existe]**         | `http://localhost:11434/v1` con `ollama`; ninguna con `openai` | Usa el valor por defecto; una cadena que no es URL falla al arrancar | URL base del proveedor (decisión 46). Con `ollama`, `OPENAI_API_KEY` es opcional
