...
import { ErrorInfraestructuraModelo } from '../modelo/errores-modelo';
...
export class FiltroErrores implements ExceptionFilter {
  ...
  private traducir(fallo: unknown): ErrorApiDto {
    ...
    if (fallo instanceof ErrorInfraestructuraModelo) {
      this.logger.warn(fallo.message);
      return {
        codigo: 'servicio-no-disponible',
        mensaje: 'El modelo de lenguaje no respondió. Intenta de nuevo en unos segundos.',
      };
    }
