 ...
 * `/api`. `AgenteModule` solo las registra con `UNIHELP_PERFIL=experimento`:
...
export class ExperimentoController {
  ...
  /** `GET /experimento/trazas/:traceId`: lo que solo B0 sabe de la ejecucion. */
  @Get('trazas/:traceId')
  async traza(@Param('traceId') traceId: string): Promise<TrazaParcialDto> {
    ...
    if (medicion === undefined) {
      throw new ErrorApi('no-encontrado', `B0 no atendió ninguna ejecución «${traceId}».`, {
        traceId,
      });
    }
