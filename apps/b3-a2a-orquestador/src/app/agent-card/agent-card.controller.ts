...
/**
 * Expone la Agent Card del orquestador en `/.well-known/agent-card.json` (HU-29).
 * `A2A_SELF_URL` es la URL con la que otros procesos lo alcanzan (en Docker, el
 * nombre del servicio); sin ella, la local.
 */
@Controller()
export class AgentCardController {
  ...
  obtenerAgentCard(): AgentCardDto {
    ...
    return tarjetaAgente('orquestador', urlBase);
  }
}
